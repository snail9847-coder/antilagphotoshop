...
class DiagnosticsTests(unittest.TestCase):
    ...
    def test_write_report_bundle_does_not_overwrite_existing_file(self) -> None:
        report = {
            ...
            "privacy_note": "note",
            "scope": [],
            "system": {"platform": "p", "python_version": "3.11", "python_executable": "python", "available_ram_mb": 1},
            "configured_photoshop": {"config_file": "c", "configured_path": None, "configured_path_exists": False},
            "disk_free_space": {},
            "logs": {"guard_log": {"text": ""}, "supervisor_log": {"text": ""}},
            "windows_application_events": {"status": "no_events", "events": [], "scan_scope": "scope"},
        }
        with tempfile.TemporaryDirectory() as temp_dir:
    ...
    def test_build_report_reads_expected_local_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(diag, "app_dir", return_value=app), mock.patch.dict(
                diag.os.environ, {"APPDATA": str(base / "AppData"), "TEMP": str(base / "Temp")}, clear=False
            ), mock.patch.object(diag, "available_ram_mb", return_value=1234), mock.patch.object(
                diag, "disk_free_bytes", return_value=5 * 1024 * 1024
            ), mock.patch.object(
                diag, "collect_windows_application_events", return_value={"status": "no_events", "events": [], "scan_scope": "scope"}
            ):
